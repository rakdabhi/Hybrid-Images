...
from proj1_code.student_code import (
    my_imfilter,
    HybridImageDataset,
    HybridImageModel,
...
def test_dataloader_get_item():
    ...
    assert torch.allclose(
        dog_img[:, 100:102, 100:102], dog_img_crop, atol=1e-3)
...
def verify_low_freq_sq_kernel_torch_manual(image1, kernel, low_frequencies) -> bool:
    ...
    if correct_sum and correct_crop:
        print('Success! Low frequency values are correct.')
        return True
    else:
        print(
            'Low frequency values are not correct, please double check your implementation.')
        return False
...
def verify_high_freq_sq_kernel_torch_manual(image2, kernel, high_frequencies) -> bool:
    ...
    if correct_sum and correct_crop:
        print('Success! High frequency values are correct.')
        return True
    else:
        print('High frequency values are not correct, please double check your implementation.')
        return False
...
def test_pytorch_low_pass_filter_square_kernel():
    ...
    img_dir = f'{ROOT}/data'
    cut_off_file = f'{ROOT}/cutoff_freq_autograder.txt'
...
def verify_low_freq_sq_kernel_pytorch(image_a, model, cutoff_sd, low_frequencies) -> bool:
    ...
    correct_sum = np.allclose(158332.06, float(
        low_freq_interior.sum()), atol=1)

    if correct_sum and correct_crop:
        print('Success! PyTorch low frequency values are correct.')
        return True
    else:
        print('PyTorch low frequency values are not correct, please double check your implementation.')
        return False
...
def verify_high_freq_sq_kernel_pytorch(image_b, model, cutoff_sd, high_frequencies) -> bool:
    ...
    if correct_sum and correct_crop:
        print('Success! PyTorch high frequency values are correct.')
        return True
    else:
        print('PyTorch high frequency values are not correct, please double check your implementation.')
        return False
